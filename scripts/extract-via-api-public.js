...
`;
  ...
  console.log('1. Writing extract helper to api.naturesmud.shop/public/unzip_next.php...');
  await saveFile(`${config.homeDir}/api.naturesmud.shop/public`, 'unzip_next.php', phpCode);

  console.log('2. Executing extract helper via HTTPS...');
  const res = await callHttps('api.naturesmud.shop', '/unzip_next.php');
  console.log('Extraction response:', res);

  console.log('3. Deleting temporary extract helper...');
  await saveFile(`${config.homeDir}/api.naturesmud.shop/public`, 'unzip_next.php', '<?php echo "cleaned"; ?>');
