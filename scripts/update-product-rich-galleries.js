...
const filePath = path.join(__dirname, '../lib/data/products.ts');
...
const regex = /export const products: Product\[\] = (\[[\s\S]*?\]);\s*export function/m;
...
const productList = JSON.parse(match[1]);

for (const prod of productList) {
  if (productImagesMap[prod.slug]) {
    const images = productImagesMap[prod.slug];
    prod.image = images[0];
    prod.images = images;
  }
}

const updatedJson = JSON.stringify(productList, null, 2);
const newContent = content.replace(regex, `export const products: Product[] = ${updatedJson};\n\nexport function`);
fs.writeFileSync(filePath, newContent, 'utf8');

console.log('Successfully updated lib/data/products.ts with rich authentic and secondary galleries!');
