...
async function main() {
  ...
  console.log('1. Uploading master database sync script to api.naturesmud.shop/public...');
  await uploadFile(localPhpPath, `${config.homeDir}/api.naturesmud.shop/public`, 'sync_db_catalog.php');
  ...
  console.log('3. Cleaning up temporary sync script...');
  const blankPath = path.join(__dirname, '..', 'scratch', 'blank.php');
  fs.writeFileSync(blankPath, '<?php echo "done"; ?>', 'utf8');
  await uploadFile(blankPath, `${config.homeDir}/api.naturesmud.shop/public`, 'sync_db_catalog.php');
